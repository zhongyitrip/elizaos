// 直接使用 Zerion API 查询余额（正确的认证方式）
async function queryZerionBalance() {
    ...
    try {
        // Zerion API v1 端点
        const baseUrl = "https://api.zerion.io/v1";
        
        // 查询投资组合
        console.log("\n📊 正在获取投资组合数据...\n");
        
        const portfolioUrl = `${baseUrl}/wallets/${address}/portfolio`;
        ...
        if (!portfolioResponse.ok) {
            console.error(`Portfolio API 错误: ${portfolioResponse.status} ${portfolioResponse.statusText}`);
            ...
            if (portfolioData.data?.attributes) {
                ...
                console.log(`   持仓数量: ${attrs.positions_distribution_by_type?.wallet || 'N/A'}`);
            } else {
        ...
        const positionsUrl = `${baseUrl}/wallets/${address}/positions/?filter[positions]=only_simple&currency=usd&sort=value`;
        ...
        if (!positionsResponse.ok) {
            console.error(`Positions API 错误: ${positionsResponse.status} ${positionsResponse.statusText}`);
...
// 执行查询
queryZerionBalance().catch(console.error);
