    ...
    if (child.stdout) {
      ...
      try {
        while (true) {
          ...
          if (done) break;
          if (!isShuttingDown) {
            process.stdout.write(`[CLI-BUILD] ${decoder.decode(value)}`);
          }
        }
      } catch {
        // Stream ended
    ...
    if (child.stderr) {
      ...
      try {
        while (true) {
          ...
          if (done) break;
          if (!isShuttingDown) {
            process.stderr.write(`[CLI-BUILD] ${decoder.decode(value)}`);
          }
        }
      } catch {
        // Stream ended
      ...
      if (buildChild.stdout) {
        ...
        try {
          while (true) {
            const { done, value } = await reader.read();
            if (done) break;
            process.stdout.write(`[REBUILD] ${decoder.decode(value)}`);
          }
        } catch {
          // Stream ended
        }
      ...
      if (buildChild.stderr) {
        ...
        try {
          while (true) {
            const { done, value } = await reader.read();
            if (done) break;
            process.stderr.write(`[REBUILD] ${decoder.decode(value)}`);
          }
        } catch {
          // Stream ended
        }
...
// Files to ignore in the watcher (auto-generated files that cause rebuild loops)
// version.ts is generated by generate-version.ts during build and contains BUILD_TIME that changes every build
/** @type {Set<string>} */
const ignoredFiles = new Set(['version.ts', 'version.js']);
...
function startFileWatcher() {
  ...
  log('WATCH', `Ignoring auto-generated files: ${[...ignoredFiles].join(', ')}`);
  ...
  watchDirs.forEach((dir, index) => {
    ...
    try {
      const watcher = watch(dir, { recursive: true }, (eventType, filename) => {
        ...
        // Only watch TypeScript files, and ignore auto-generated files
        if (filename && (filename.endsWith('.ts') || filename.endsWith('.tsx'))) {
          // Skip auto-generated files that would cause rebuild loops
